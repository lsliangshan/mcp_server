...
import { formatMorePositionsUrl, formatRequestParams, formatResponsePositionsTemplate, formatSalary, getReponseTemplate, getWorkExpCodeByYear, translateToPositions, } from "../utils/zhaopin.js";
...
server.addTool({
    ...
    execute: async (args) => {
        ...
        if (resumeDetail.unifiedPurposes &&
            ...
            \n求职意向: ${resumeDetail.unifiedPurposes
                            .map((item) => `期望行业: ${item.pnewPreferredIndustryTranslation} - 期望城市: ${item.preferredLocationTranslation} - 期望薪资: ${item.preferredSalaryMin}-${item.preferredSalaryMax} 元/月 - 期望工作性质: ${item.preferredJobNatureTranslation}`)
                            .join("\n")}
            \n教育经历: ${resumeDetail.educationExperiences
                            .map((item) => `学校名称: ${item.eduSchoolName} - 学历: ${item.eduBackgroundTranslation} - 专业: ${item.eduMajorV} - 在校时间: ${dateFormat(item.eduStartDate)}至${item.eduEndDate == 0 ? "至今" : dateFormat(item.eduEndDate)}`)
                            .join("\n")}
            \n工作/实习经历: ${resumeDetail.workExperience
                            .map((item) => `公司名称: ${item.companyName} - 所属行业: ${item.wnewIndustryTranslation} - 职位名称: ${item.title} - 拥有技能: ${item.skillTagList
                            .map((skill) => skill.name)
                            .join(",")} - 当前月薪: ${item.realSalary} 元/月 - 在职时间: ${dateFormat(item.startDate)} - 离职时间: ${item.endDate == 0 ? "至今" : dateFormat(item.endDate)} - 工作描述或内容: ${item.workDesc}`)
                            .join("\n")}
            \n项目经历: ${resumeDetail.projectExperiences
                            .map((item) => `项目名称: ${item.proExpProjectName} - 项目开始时间: ${dateFormat(item.proExpStartDate)} - 项目结束时间 ${item.proExpEndDate == 0
                            ? "至今"
                            : dateFormat(item.proExpEndDate)} - 项目描述: ${item.proExpProjectDesc}`)
                            .join("\n")}
...
server.addTool({
    ...
    description: "<for-assistant>当用户需要推荐职位时，不要调用此工具，应该调用 recommendPositions 工具。</for-assistant>搜索职位。搜索条件包括：职位或公司名称、职位类别、公司行业、工作地点、地铁沿线、薪资范围、学历要求、工作经验、职位类型、公司性质、公司规模",
    parameters: z.object({
        at: z.string().optional(),
        rt: z.string().optional(),
        /// 搜索关键词，职位或公司名称
    ...
    execute: async (args) => {
        ...
        const { resumeNumber } = await getResumeNumber(args.at
            ? {
                at: args.at,
                rt: args.rt,
            }
...
server.addTool({
    ...
    parameters: z.object({
        ...
        resumeIndex: z
            ...
            .optional()
            .default(1)
            .describe("简历索引，指定索引的简历，会被用于职位投递。索引从 1 开始，默认是第一份简历。"),
    }),
    execute: async (args) => {
        if (!args.at || args.at == "undefined") {
            delete args.at;
        ...
        if (selectedIndex >= resumes.length) {
            return {
                content: [
                    {
                        type: "text",
                        text: `准备使用第 ${selectedIndex + 1} 份简历投递，但是用户只有 ${resumes.length} 份简历，提示用户选择其他简历进行投递`,
                    },
                ],
            };
        }
        ...
        if (!selectedResume.cnCompleted) {
            return {
                content: [
                    {
                        type: "text",
                        text: `准备使用第 ${selectedIndex + 1} 份简历投递，但是该简历不完整，提示用户先完善您的简历。每次都要返回完善简历的地址（<a href="https://i.zhaopin.com/resume" target="_blank" class="primary-color">完善简历</a>）`,
                    },
                ],
            };
        }
        ...
        // 获取职位详情
        const positionDetail = await getPositionDetailBatch({
            at: args.at,
            rt: args.rt,
        ...
        // 无效的职位
        let unvalid = [];
        positionDetail.data.forEach((item) => {
            if (item.code == 200) {
        ...
        const deliveredTemplate = delivered.length > 0
            ? `\n${delivered.length} 个重复投递的职位: ${delivered
                .map((item) => `${item.companyName}-${item.positionName}-${item.salary60}（${item.number}）`)
                .join(",")}`
            : "";
        const unvalidTemplate = unvalid.length > 0
        ...
        const deliveryInfo = await deliveryPositions({
            at: args.at,
            rt: args.rt,
...
server.addTool({
    name: "getMyAtRt",
    description: "获取我的at和rt<for-assistant>用户需要推荐职位或工作时，请调用 recommendPositions 工具</for-assistant><for-assistant>用户需要搜索职位或工作时，请调用 searchPositions 工具</for-assistant>",
    parameters: z.object({
        at: z.string().optional(),
        rt: z.string().optional(),
    }),
