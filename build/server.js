...
server.addTool({
    ...
    execute: async (args) => {
        ...
        if (args.jobType) {
            params[JobSearchConditionMap.jobType] = jobTypes[args.jobType];
        }
        if (args.industry) {
            params[JobSearchConditionMap.industry] = args.industry.map((item) => industries[item]).filter((item) => item).join(';');
        }
        ...
        if (args.subway) {
            // TODO: 地铁沿线
            params[JobSearchConditionMap.subway] = subways[args.subway];
        }
        if (args.subwayStation) {
            params[JobSearchConditionMap.subwayStation] = subwayStations[args.subwayStation];
        }
        if (args.salaryType) {
