...
server.addTool({
    ...
    description: "获取我的求职意向，包括期望工作地点、期望工作行业、期望工作性质、期望薪资",
    parameters: z.object({
        at: z.string().optional(),
        rt: z.string().optional(),
    }),
...
server.addTool({
    ...
    execute: async (args) => {
        ...
        if (resumeDetail.UnifiedPurpose && resumeDetail.UnifiedPurpose.length > 0) {
            ...
            \n教育经历: ${resumeDetail.EducationExperience.map((item) => `学校名称: ${item.eduSchoolName} - 学历: ${item.eduBackgroundTranslation} - 专业: ${item.eduMajorV} - 在校时间: ${item.eduStartDateFormat.split(' ')[0]}至${item.eduEndDateFormat.split(' ')[0]}`).join('\n')}
            \n工作/实习经历: ${resumeDetail.WorkExperience.map((item) => `公司名称: ${item.companyName} - 所属行业: ${item.wnewIndustryTranslation} - 职位名称: ${item.title} - 拥有技能: ${item.skillTagList.map((skill) => skill.name).join(',')} - 当前月薪: ${item.realSalary} 元/月 - 在职时间: ${item.startDateFormat.split(' ')[0]} - 离职时间: ${item.endDateFormat.split(' ')[0]} - 工作描述或内容: ${item.workDesc}`).join('\n')}
            \n项目经历: ${resumeDetail.ProjectExperience.map((item) => `项目名称: ${item.proExpProjectName} - 项目开始时间: ${item.proExpStartDateFormat.split(' ')[0]} - 项目结束时间 ${item.proExpEndDateFormat.split(' ')[0]} - 项目描述: ${item.proExpProjectDesc}`).join('\n')}
            ...
            \n如果用户只想获取某一份教育经历，或工作/实习经历，或项目经历，则只返回该份教育经历，或工作/实习经历，或项目经历，不要返回其他内容，并输出为表格形式，表格使用 HTML 标签。
