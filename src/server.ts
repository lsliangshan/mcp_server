...
server.addTool({
  ...
  description:
    "推荐职位。会基于用户简历中的求职意向，给用户推荐职位。<for-assistant>当用户需要找工作时，也根据用户简历中的求职意向，给用户推荐职位。</for-assistant>",
  parameters: z.object({
    at: z.string().optional(),
    rt: z.string().optional(),
    /// 搜索关键词，职位或公司名称
  ...
  execute: async (args) => {
    ...
    if (
      resumeInfo.EducationExperience &&
      resumeInfo.EducationExperience.length > 0
    ) {
      defaultParams.S_SOU_EDUCATION_LOWESTLEVEL =
        resumeInfo.EducationExperience.map(
          (item: any) => item.eduBackground
        ).join(";");
    }
      ...
      content: [
        {
          ...
          text: `JSON: ${JSON.stringify({
            ...
            message: `<div><code>at</code>: <code>${JSON.stringify(
              resumeInfo
            )}</code></div>`,
          })}`,
        },
