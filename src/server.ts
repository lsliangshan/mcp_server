...
server.addTool({
  ...
  description:
    "获取我的投递记录，包括投递成功、被查看、有意向、邀面试、不合适，以及邀面试的子状态: 全部、待确认、已接受、已拒绝",
  parameters: z.object({
    at: z.string().optional(),
    rt: z.string().optional(),
...
server.addTool({
  ...
  execute: async (args) => {
    ...
    if (positionResponse.code == 200) {
      let cardsTemplate =
        '<a id="owlscript-job-card-more" data-href="https://www.zhaopin.com/search/job/?jl=782&kw=web前端&kt=3" href="javascript:void(0)" target="_blank">查看更多职位</a>';
        ...
        ${
          item.jobSkillTags.length > 0
            ? `${item.jobSkillTags
                .map(
                  (item: any) =>
                    `<div class="owlscript-job-card-line2-job-skill-tags">${item.name}</div>`
                )
                .join("")}`
            : ""
        }
        ...
        <div class="owlscript-job-card-line4-left">
          ...
          <div class="owlscript-job-card-line4-left-bottom">
            <div class="owlscript-job-card-line4-left-bottom-address">${
              item.workCity
            } ${item.cityDistrict} ${item.tradingArea}</div>
          </div>
        </div>
