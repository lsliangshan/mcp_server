export function collectIndustryNames(data: any) {
  const result: any = {};

  const traverse = (nodes: any) => {
    for (const node of nodes) {
      // 跳过已删除项和"不限"项
      if (node.deleted || node.name === '不限') continue;
...
export function collect2(data: any) {
  const result: any = {};
  ...
  for (const node of data) {
    ...
    if (node.sublist?.length) {
      for (const subNode of node.sublist) {
        if (subNode.deleted || subNode.name === '不限') continue;
...
export function collect3(data: any) {
  const result: any = {};
...
function generateMetroMap(data: any) {
  const result: any = {};
  // 遍历第一级（城市）
  for (const level1 of data) {
    if (level1.deleted || level1.name === '不限') continue;
    // 遍历第二级（地铁线）
    for (const level2 of level1.sublist || []) {
      if (level2.deleted || level2.name === '不限') continue;
      // 遍历第三级（站点）
      for (const level3 of level2.sublist || []) {
        if (level3.deleted || level3.name === '不限') continue;
        // 构造键和值
