...
server.addTool({
  ...
  execute: async (args) => {
    ...
    //   ],
    // };
    const deliveredList: any = await getJobDelivered({
      at: args.at,
      rt: args.rt,
    ...
    if (deliveredList.code == 200) {
      const rangeTime = getRangeTimeByTimeType(
        args.date || args.time || "last30Days"
      );

      const filteredData = deliveredList.data.filter((item: any) => {
