...
/** 自然年范围 */
function getYearRange(offsetYears: number = 0, baseDate: Date = new Date()) {
  const date = new Date(baseDate);
...
// 计算自然周范围（内部实现）
function calculateWeekRange(date: Date) {
...
// 计算自然月范围（内部实现）
function calculateMonthRange(date: Date) {
...
// 计算自然年范围（内部实现）
function calculateYearRange(date: Date) {
...
// 新增：计算自然季度范围（内部实现）
function calculateQuarterRange(date: Date) {
